# 使用testSetRBF.txt和testSetRBF2.txt，前者作为训练集，后者作为测试集
...
def testRbf(k1=1.3):
    ...
    Returns:
        无
    """
    dataArr,labelArr = loadDataSet('./my_datasets/SVM/testSetRBF.txt')                        # 加载训练集
    ...
    dataArr, labelArr = loadDataSet('./my_datasets/SVM/testSetRBF2.txt')    # 加载测试集
    errorCount = 0
    datMat = np.mat(dataArr)
    labelMat = np.mat(labelArr).transpose()
    m, n = np.shape(datMat)
