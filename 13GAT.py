...
class GraphAttentionLayer(nn.Module):
    ...
    def forward(self, h, adj):
        ...
        zero_vec = -9e15*torch.ones_like(e)  # 将没有连接的边设置为负无穷
        attention = torch.where(adj > 0, e, zero_vec)  # [N,N], 如果adj>0则返回e, 反之返回zero_vec
        # 表示如果邻接矩阵元素大于0时，则两个节点有连接，该位置的注意力系数保留
        # 否则需要mask设置为非常小的值，因为softmax的时候这个最小值会不考虑
        attention = F.softmax(attention, dim=1)  # softmax形状保持不变[N,N]，得到归一化的注意力权重
        attention = F.dropout(attention, self.dropout, training=self.training)  # dropout,防止过拟合
        h_prime = torch.matmul(attention, Wh)  # [N,N].[N,out_features]=>[N,out_features], 论文中的公式(4)
